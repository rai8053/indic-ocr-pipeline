...
def generate_report(pages: list[dict], output_path: Path, usage: dict | None = None):
    ...
    for p in pages:
        ...
        for ck, cv in sorted(checks.items()):
            if isinstance(cv, dict):
                status_str = f"{cv.get('valid',0)}/{cv.get('valid',0)+cv.get('invalid',0)}" if cv.get('valid',0)+cv.get('invalid',0) > 0 else "N/A"
