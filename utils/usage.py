...
PRICING = {
    ...
    "openrouter": {
        "description": "OpenRouter Gemini 2.0 Flash",
        "free_tier": True,
    },
}
...
def _compute_cost(provider: str, monthly_used: int, pages: int = 0,
...
class UsageTracker:
    ...
    def record_request(self, provider: str, model: str = "", success: bool = True,
    ...
    def _update_period(self, bucket: dict, period_id: str, provider: str, agg: dict):
    ...
    def _prune_old(self, aggs: dict, period: str, max_count: int):
        keys = sorted(aggs.get(period, {}).keys())
        if len(keys) > max_count:
            for k in keys[:-max_count]:
                del aggs[period][k]

    def fetch_openrouter_official_usage(self, api_key: str) -> dict | None:
        if not api_key:
            return None
        try:
            resp = requests.get(
    ...
    def dashboard(self, settings: dict | None = None) -> dict:
        now_ts = time.time()
