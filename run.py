...
from core.terminal import banner, rule, info, ok, warn, err, bold, raw
from core.config import QUOTA_STATE_FILE
from utils.usage import UsageTracker, PROVIDER_INFO
...
def show_quota_monitor(settings):
    from core.config import QUOTA_STATE_FILE, OPENROUTER_API_KEY
    tracker = UsageTracker(QUOTA_STATE_FILE)
    data = tracker.dashboard(settings)
