"""Pipeline runner — main entry point for the OCR annotation workflow."""
...
def process_pdf(
    ...
    """Run the full OCR annotation pipeline on a single PDF.

    Stages:
    1. **PDF → Images** — Render PDF pages to JPEG at given DPI.
    2. **Picture Detection** — Find embedded/CV-detected picture regions.
    3. **Preprocessing** — Optional deskew/denoise/contrast (OpenCV).
    4. **Google Vision OCR** — Extract paragraph-level text and bounding boxes.
    5. **LLM Proofreading** — Send batches to vision/text LLM with failover.
    6. **Auto Relations** — Fill missing block_relations for Level 4.
    7. **Validation & Scoring** — RFQ quality assessment.
    8. **QA Overlays** — Visual overlay images (optional).
    9. **HTML Report** — Quality summary report (optional).
    10. **ZIP Export** — Submission archive (optional).

    Args:
        pdf_path: Path to the source PDF.
        lang: Target language key (e.g. ``"odia"``). Auto-detected if empty.
        out_dir: Root output directory.
        dpi: Render DPI for page images. Default 150.
        jpeg_quality: JPEG quality 1-100. Default 60.
        provider: Primary LLM provider name. Default ``"gemini"``.
        batch_size: Pages per API request. Default 1.
        create_zip: Whether to create a ZIP archive.
        max_samples: Max pages in ZIP (0 = all).
        validate: Whether to validate and score annotations.
        level: Annotation level (3 or 4). Default 3.
        preprocess: Whether to apply OpenCV preprocessing.
        qa: Whether to generate visual QA overlay images.
        create_report: Whether to generate HTML quality report.
        max_pages: Process only the first N pages (0 = all).
    """
    global _tracker
    ...
    _tracker = UsageTracker(QUOTA_STATE_FILE)
    _usg = _tracker
    ...
    for img_path in pending:
        ...
        try:
            vision_result = run_vision_ocr(img_path, language_hints=hints, usage_recorder=_usg)
        except Exception as e:
            plog.error(f"Vision failed: {e}")
            plog.end_stage("vision_ocr")
            continue
        plog.end_stage("vision_ocr")
    ...
    for i in range(0, len(ocr_items), batch_size):
        ...
        plog.log(
            f"[3/5] Proofread ({provider}, batch {len(chunk)}): "
            f"{', '.join(p.name for p in chunk_paths)}"
        )
        ...
        try:
            ...
            for img_path, page_result in zip(chunk_paths, pages_out):
                ...
                if page_result.get("annotation_quality") == "degraded_text_only_fallback":
                    plog.log(
                        f"      [WARN] {img_path.stem}: Level 4 requested but fell back to "
    ...
    # Stage 4: Validation + Scoring + QA
    jsons = sorted(json_dir.glob("*.json"))
    all_validations: list[tuple[Path, dict, dict]] = []
    all_overlays: list[Path] = []

    if validate or qa or create_report:
        plog.start_stage("validation")
        plog.log("[4/5] Analyzing annotations...")
        all_validations = validate_and_score_pages(jsons)

        for j, r, s in all_validations:
            status = "PASS" if r["valid"] else "FAIL"
            detail = f" ({len(r['errors'])} errors)" if not r["valid"] else ""
            plog.log(
                f"      {j.name}: {status}{detail} | "
                f"Scores: OCR={s['ocr']}% RO={s['reading_order']}% Overall={s['overall']}%"
            )
            ...
            if qa:
                ...
                if src_img.exists():
                    overlay_path = (
                        qa_dir / f"{j.stem}_overlay.jpg"
                        if qa_dir
                        else out_dir / "qa" / f"{j.stem}_overlay.jpg"
                    )
    ...
    if _usg:
        ...
        for prov, pd in _d.get("providers", {}).items():
            ...
            lt = pd["lifetime"]
            plog.log(
                f"  {pd['label']:20s}: {t['requests']:3d} today, "
...
def main() -> None:
    """CLI entry point for the pipeline (argument-parsing wrapper)."""
