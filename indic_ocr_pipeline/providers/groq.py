"""Groq LLM provider wrapper (text-only fallback)."""
...
def run_groq_proofread_batch(
    ...
    Args:
        image_paths: Page image paths (used for count only).
        pages_blocks: Raw OCR blocks per page.
        level: Annotation level (3 or 4).
        usage_recorder: Optional UsageTracker for recording.

    Returns:
        Parsed page annotations with ``annotation_quality = "degraded_text_only_fallback"``
        for level >= 4.
    ...
    try:
        ...
        if usage_recorder:
            usage_recorder.record_request(
                "groq", success=True, latency_ms=lat,
        ...
        if usage_recorder:
            usage_recorder.record_request(
                "groq", success=False, latency_ms=0,
