...
from core.config import CLASS_COLORS, VALID_CLASSES


def draw_overlay(image_path: Path, annotation_path: Path, output_path: Path,
                 show_reading_order: bool = True, show_relations: bool = True) -> Path:
    ...
    if show_reading_order and reading_order and len(reading_order) > 1:
        for k in range(len(reading_order) - 1):
            ...
            cv2.putText(overlay, str(k + 1), (mid_x, mid_y),
    ...
    if show_relations and relations:
        for rel in relations:
            ...
            cv2.putText(overlay, rtype, ((sx + tx) // 2, (sy + ty) // 2 - 5),
                        cv2.FONT_HERSHEY_SIMPLEX, 0.35, (0, 255, 255), 1, cv2.LINE_AA)

    legend_x = 10
    legend_y = h - 30 * len(VALID_CLASSES) - 10
    cv2.rectangle(overlay, (legend_x - 5, legend_y - 5),
                  (legend_x + 160, legend_y + 30 * len(VALID_CLASSES) + 5), (0, 0, 0), -1)
    cv2.putText(overlay, "Legend:", (legend_x, legend_y + 15),
    ...
    for idx, cls in enumerate(VALID_CLASSES):
        ...
        cv2.putText(overlay, cls, (legend_x + 20, ly),
